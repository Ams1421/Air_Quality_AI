...
- **Classify** hourly AQI into 6 health categories (Good → Hazardous)
...
│
├── app.py                       # Complete app: ML pipeline + backend + frontend (single file)
├── generate_report.py           # Regenerates report.html after a new run
├── requirements.txt             # Python dependencies
├── README.md                    # This file
├── report.html                  # Self-contained HTML report (794 KB, all charts embedded)
...
### 2. Run Pipeline + Launch Dashboard

```bash
python app.py
```

This runs the full ML pipeline first (data → EDA → training → evaluation → saves models/charts), then starts the web server.

To skip retraining and use existing saved models:
